  ...
  if [[ "$DontUpdate" == false ]]; then
    ...
    if [[ "$SkipRecreate" == true ]]; then
      ...
      for i in "${SelectedUpdates[@]}"; do
        ...
        # Handle Windows paths if needed (convert C:\Users... to /c/Users... or /mnt/c/Users...)
        if [[ "$ContPath" == *:* ]]; then
            if command -v wslpath &>/dev/null; then
                ContPath=$(wslpath -u "$ContPath")
            elif command -v cygpath &>/dev/null; then
                ContPath=$(cygpath -u "$ContPath")
            else
                # Fallback: check if /mnt exists (WSL) or default to /c/ (Git Bash/MinGW)
                if [[ -d "/mnt/c" ]] || [[ -d "/mnt/d" ]]; then
                    ContPath=$(echo "$ContPath" | sed -e 's/\\/\//g' -e 's/^\([a-zA-Z]\):/\/mnt\/\L\1/')
                else
                    ContPath=$(echo "$ContPath" | sed -e 's/\\/\//g' -e 's/^\([a-zA-Z]\):/\/\L\1/')
                fi
            fi
        fi
        
        cd "$ContPath" || { printf "\n%bPath error - skipping%b %s (Path: %s)" "$c_red" "$c_reset" "$i" "$ContPath"; continue; }
        
        # Sanitize Windows paths in ConfigFiles (handle comma separation)
        if [[ "$ContConfigFile" == *:* ]]; then
             CleanedConfs=""
             IFS=',' read -ra ADDR <<< "$ContConfigFile"
             for conf in "${ADDR[@]}"; do
                 if [[ "$conf" == *:* ]]; then
                    if command -v wslpath &>/dev/null; then
                        conf=$(wslpath -u "$conf")
                        ...
                        conf=$(cygpath -u "$conf")
                         ...
                         if [[ -d "/mnt/c" ]] || [[ -d "/mnt/d" ]]; then
                            conf=$(echo "$conf" | sed -e 's/\\/\//g' -e 's/^\([a-zA-Z]\):/\/mnt\/\L\1/')
                        else
                            conf=$(echo "$conf" | sed -e 's/\\/\//g' -e 's/^\([a-zA-Z]\):/\/\L\1/')
                        fi
                    fi
                 fi
                 if [[ -z "$CleanedConfs" ]]; then CleanedConfs="$conf"; else CleanedConfs="$CleanedConfs,$conf"; fi
             done
             ContConfigFile="$CleanedConfs"
        fi
